...
6. 親: `Pelvis`
7. 初期角度: 座った状態にするため、**90度** 曲げたいところですが、初期姿勢はモデル作成時の配置で決まります。
...
4. **Y座標の符号を反転**させます（例: Y=-0.08 → **Y=0.08**）。
   - `LHipJoint`: Y=0.08
   - `LThigh`: Y=0.08
   - `LKneeJoint`: Y=0.08
   - ...
5. **Orientation はそのまま**でOKです（左右で同じ値）。
   - 回転軸（Y軸）は体の左右を貫く共通の軸なので、左右で同じ Orientation 値にしておくと、同じ角度を与えた時に対称的な動きになります。
...
> [!IMPORTANT]
> **この段階ではまだ立ち上がりません！** ジョイント設定だけでは動作しません。次のセクション4でスクリプトを追加してはじめて動きます。

### 3.3 動作確認（この段階での挙動）

この段階でシミュレーションを実行すると、**Target angle = 0** を維持しようとします：
- 重力で姿勢が崩れそうになっても、各関節が0度を維持しようとトルクを発生します
- 正しく設定されていれば、**座った姿勢のまま安定** するはずです

### 3.4 トラブルシューティング

**❌ 関節がバラバラになる（股関節が骨盤から離れるなど）:**
1. **親子関係を確認**: Scene hierarchyで以下の構造になっているか確認
   ```
   Pelvis
    ├── RHipJoint → RThigh → RKneeJoint → RShank → RAnkleJoint → RFoot
    └── LHipJoint → LThigh → ...
   ```
2. **ジョイントの Mode を確認**: 各ジョイントを選択し、Scene Object Properties で **Mode** が **Dynamic mode** になっていることを確認（Kinematic modeだとバラバラになります）

**❌ 膝が変な方向に曲がる:**
- ジョイントの **Orientation** が正しいか確認してください（セクション2参照）
