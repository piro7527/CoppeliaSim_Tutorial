...
スクリプト内の `timer` と実際の物理減少（イベント）にはズレがあります。これを理解しておくとパラメータ調整に役立ちます。

| 時刻 (`timer`)   | スクリプト上の処理            | 物理的な挙動 (Event)                                                                             |
| :--------------- | :---------------------------- | :----------------------------------------------------------------------------------------------- |
| **5.50s** (`t3`) | **動作開始** (Motion Start)   | 関節が動き始める（まだお尻は座面に残っている）。                                                 |
| **~6.50s**       | （中間地点）                  | **殿部離床 (Seat-off) & 最大トルク (Peak Torque)**<br>重心が浮き上がり、最も力が必要になる瞬間。 |
| **7.50s** (`t4`) | **動作完了** (Stand Complete) | 起立姿勢で静止。                                                                                 |

> **Point**: 「5.5秒」はあくまで「動き始め」の命令です。「6.5秒」付近でトルクがピークになるのは、重心移動と持ち上げ動作が重なるためで、正常な挙動です。
...
### 📊 実験結果の例
実際にデータを取得してみると、以下のような違いが見られます。

*   **Assist Force = 150N**: 膝関節トルクは比較的低く抑えられています。バランス良く立てています。
*   **Assist Force = 120N**: **130サンプル目（約6.5秒、殿部離床の瞬間）** 付近で、膝関節トルク（Knee Torque）が **顕著に大きく** なります。
    *   これはアシストが減った分、ロボット自身が「踏ん張る」必要があることを物理的に示しています。
