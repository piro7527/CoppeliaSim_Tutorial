...
1.  Scene Hierarchy で **`RFoot`** を選択します。
2.  **View** > **Apply Customization** (または Geometry properties) でサイズを変更します：
    *   Size: **X=0.30, Y=0.30, Z=0.05** （幅と長さを広げます）
    *   **幅（Y）**: 左右のバランスを安定させます（擬似両足）。
    *   **長さ（X=0.30）**: 前後の支持基底面を広げ、立ち上がり時の転倒を防ぎます。
...
### 5-2. 重心が後ろに倒れる場合
*   **前傾（Lean）を深くする**: 変数 `leanHip`（股関節屈曲）を **140度**（実績値）まで深くしてみてください。お辞儀が深くなるほど重心は前方に移動します。
...
手順1-4で設定した `RFoot` の Xサイズを **0.30**（実績値）に設定してください。
支持基底面が前後に広がるため、立ち上がりの勢いで前に倒れそうになっても耐えられるようになります。

### 5-6. 立ち上がり後に前方へ倒れる場合（Forward Fall Prevention）
`RFoot X=0.30`, `leanHip=140°` で立ち上がりには成功するが、勢い余って前方に倒れてしまう場合があります。

#### 根本原因：前傾中のつま先浮き（Forefoot Lift-off）
前方転倒の多くは、**Phase 1（前傾）中に足のつま先が浮いてしまう** ことが根本原因です。
```
原因の連鎖：
  leanAnkle = 40°（強い背屈）
  → 重心がまだ足の上に十分乗っていない状態で足首トルクがかかる
  → 反力で前足部（つま先）が持ち上がる
  → 支持基底面が踵側に縮小
  → 伸展時に前方へ容易に転倒
```
> **注意**: `t2`（伸展時間）を延ばすと、前傾姿勢での滞在時間が長くなり、つま先浮きが**悪化**します。`t2 = 4.0` のまま維持してください。

#### ① leanAnkle を小さくする（最も効果的）
前傾中の足首背屈を抑えることで、つま先浮きを防ぎます。
```lua
local leanAnkle = 20 * math.pi / 180  -- 40 → 20〜25 に低減
```
背屈が浅くなると前傾の深さが減りますが、`leanHip = 140°` が十分に深いため、重心は足の上に到達できます。**5度ずつ** 下げて試してください。

#### ② 足を後方に配置する（Posterior Foot Placement）
足（`RFoot`）の初期位置を椅子の方へ引き、膝の真下〜やや後ろに配置します。
```
RFoot Position X = -0.10 〜 -0.15（現在値から後方へ）
```
足が膝の下にあると、前傾時に重心が自然と足の上に乗り、つま先浮きが起きにくくなります（手順1-1で椅子の下にスペースを確保済み）。

#### ③ 足の質量をさらに増やす
質量を増やすことで、足首トルクに対する足のリフトオフ抵抗が上がります。
```
RFoot Mass = 3.0〜5.0 kg（現在 2.0kg から増量）
```
物理的にはリアルではありませんが、シミュレーション安定化テクニックとして有効です。

#### ④ 前傾角度を微調整する
上記①②③でまだ前方に倒れる場合は、`leanHip` を **2度ずつ** 浅くしてみてください。
```lua
local leanHip = 138 * math.pi / 180  -- 140 → 138 に微調整
```
